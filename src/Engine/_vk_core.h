...
inline bool CreateCommandPool(VkCommandPool& pool, VkDevice device, uint32_t queueFamilyIndex, VkCommandPoolCreateFlags flags = 0)
{
	...
	VK_CHECK_FALSE(vkCreateCommandPool(device, &commandPoolCreateInfo, nullptr, &pool));

	return true;
}

inline bool AllocateCommandBuffers(VkCommandBuffer& cmd, VkDevice device, VkCommandPool cmdPool)
{
	...
	VK_CHECK_FALSE(vkAllocateCommandBuffers(device, &allocInfo, &cmd));
...
{
	VkCommandBuffer cmd{};
	if (!AllocateCommandBuffers(cmd, device, cmdPool))
	{
		core::Fatal("vkAllocateCommandBuffers failed.");
		// TODO: return null
	}
	...
	auto result = vkBeginCommandBuffer(cmd, &beginInfo);
